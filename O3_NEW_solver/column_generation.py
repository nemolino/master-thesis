...
class Column:
    ...
    def __init__(self, col_cost, col_q_core, col_q_bandwidth):
...
class Master:
    ...
    def __init__(self, data, dummy_columns_costs, logger=None):

        self.data = data

        self.COLUMNS_POOL = None
        self.initialize_columns_pool(dummy_columns_costs)
        self.logger = logger
        self.z = None
        self.model = None
    
    def initialize_columns_pool(self, dummy_columns_costs):
        ...
        for n in range(N): 
            ...
            dummy_col = Column(
                dummy_col_cost, dummy_col_q_core, dummy_col_q_bandwidth
            )
    ...
    def add_column(self, n, new_column):
        ...
        if new_column in self.COLUMNS_POOL[n]:
            raise RuntimeError("Master add_column : added duplicate column for {n}-th app")
    ...
    def build_and_optimize(self):
        ...
        if model.Status != GRB.OPTIMAL:
            raise RuntimeError("CG Master build_and_optimize : master status diverso da optimal")
        

        #print(f"z* = {model.ObjVal} , master build in {end-start:.3f} s and solve in {model.Runtime:.3f}\n")
        LOG.info(f"z* = {model.ObjVal} , master build in {end-start:.3f} s and solve in {model.Runtime:.3f}")
        LOG.info("")
    ...
    def print_opt_sol(self):
        ...
        for n in range(N): 
            ...
            for p in range(len(COLUMNS_POOL[n])):
                if z[n][p].X > 0:
                    print(f"\tpattern {p:4d} : {z[n][p].X:.3f}")
    ...
    def print_opt_sol_discrete(self):
        ...
        if infeasible:
            print('\033[91m' + f"INFEASIBLE discrete RMP : we are still using a dummy column" + '\033[0m')
            LOG.info("INFEASIBLE discrete RMP : we are still using a dummy column")
...
class Pricing:
    ...
    def optimize(self, λ_positive, μ_positive, η_n):
        ...
        new_col = Column(
            new_col_cost, new_col_q_core, new_col_q_bandwidth
        )
...
class ColumnGeneration:
    ...
    def execute(self):
        ...
        while True:
            ...
            λ_positive, μ_positive, η, master_obj_val = master.build_and_optimize()
            ...
            for n in range(N):
                ...
                sum_reduced_costs += reduced_cost # credo che vada spostato più giù
            ...
            for n in range(N):
                if columns_to_add[n] is not None:
                    master.add_column(n,columns_to_add[n])
        ...
        infeasible = master.print_opt_sol()
        if not infeasible:
