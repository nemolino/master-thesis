...
class InfeasiblePricing(Exception):
    ...
    def __init__(self, message="InfeasiblePricingException"):
...
class Master:
    ...
    # done
    def add_column(self, n, new_column):
    ...
    def optimize(self):
        ...
        #print(f"z* = {model.ObjVal} , master build in {end-start:.3f} s and solve in {model.Runtime:.3f}\n")
    ...
    """
    @staticmethod
    def print_positive_dual_values_util(λ,μ,η):
        print("Positive dual values :")
        for k,v in λ.items():
            if v > 0:   print(f"\tλ {k} = {v}")
            else:       assert abs(v) < 0.001
        for k,v in μ.items():
            if v > 0:   print(f"\tμ {k} = {v}")
            else:       assert abs(v) < 0.001
        for i,v in enumerate(η):
            if v > 0:   print(f"\tη_{i} = {v}")
            else:       assert abs(v) < 0.001
    """
...
class Pricing:
    ...
    """
    def get_reduced_cost(self, x,λ,μ,η_n):

        (
            I,A,T,D,R,a,a_format2,a_format3,
            Q_nodes,q_microservices,b_microservices,
            Q_links,q_connections,b_connections,b_connections_format2,c
        ) = self.data

        c = (
            gp.quicksum(c[i] * x[u,i] for u in T for i in I) 
            - η_n
            + gp.quicksum(
                q_microservices[(u,k)] * λ[(i,k)] * x[u,i] 
                    for k in R 
                        for i in I
                            for u in T
            )
            + gp.quicksum(
                #q_connections[((u,v),k)] * a[((i,j),(l,m))] * μ[((l,m),k)] * x[u,i] * x[v,j] 
                q_connections[((u,v),k)] * μ[((l,m),k)] * x[u,i] * x[v,j]
                    for k in R 
                        for (l,m) in A
                            for i,j in a_format2[(l,m)]
                            # for i in I 
                            #     for j in I 
                                    for (u,v) in D
            )
    ...
    def execute(self):
        ...
        for n in range(N):
            ...
            try:
                ...
                print('\033[91m' + f"INFEASIBLE INSTANCE : (on building) there is a microservice you cannot place in any node" + '\033[0m')
        ...
        for n in range(N):
            ...
            try:
                ...
                print('\033[91m' + f"INFEASIBLE INSTANCE : {n}-th pricing infeasible with dual values = 0, meaning that you cannot map {n}-th application" + '\033[0m')
            ...
            LOG.info(f"{n}-th heuristic pricing build")
        ...
        for n in range(N):
            ...
            print(f"app {n} ")
            start = perf_counter()
            feasible_columns = heuristic_pricings[n].generate_feasible_columns_semigreedy() 
            end = perf_counter()
            print(f"generated {len(feasible_columns)} feasible columns - time {end-start:.2f} s, ", end='')
            
            costs = []
            actually_added = 0
            for sol in feasible_columns:

                #print(sol[0], sol[1].cur_z)
                #sol = sol[1]

                new_col_cost = sum(c[sol.cur_x[u]] for u in range(APP_n_T))
                new_col_q_core = np.zeros(I, dtype=np.int32)
                ...
                new_column = Column(
                    new_col_cost, new_col_q_core, new_col_q_bandwidth, sol.cur_x
                )

                costs.append(new_col_cost)
                added = master.add_column_possibly_duplicate(n,new_column)
                if added:
                    actually_added += 1

            print(f"actually added {actually_added}/{len(feasible_columns)} , min_cost = {min(costs) if len(costs) > 0 else None} , avg_cost = {np.mean(costs) if len(costs) > 0 else None}")
            print()
        ...
        while True:
            ...
            while True:
                ...
                LOG.info(f"iteration {iterations_count}")
                LOG.info("")
                ...
                try:
                    ...
                    return dual_bound, primal_bound
                ...
                print(f"-- {'EXACT' if EXACT else ''} {heuristic_pricings_consecutive_fails}")
                for n in range(N):
            ...
            used_nodes = [[None] * APP[n]["T"] for n in range(N)]
            for n in range(N): 
                for p in range(len(master.COLUMNS_POOL[n])):
                    if master.z[n][p].X > 0:
                        for u in range(APP[n]["T"]):
                            i = master.COLUMNS_POOL[n][p].original_x[u]
                            if used_nodes[n][u] is None:
                                used_nodes[n][u] = set()
                            used_nodes[n][u].add(i)
            ...
            if not infeasible:
                ...
                for n in range(N):
                    ...
                    print(f"app {n} ")
                    start = perf_counter()
                    feasible_columns = heuristic_pricings[n].generate_feasible_columns_repair(used_nodes[n]) 
                    end = perf_counter()
                    print(f"generated {len(feasible_columns)} feasible repair columns - time {end-start:.2f} s, ", end='')
                    ...
                    print(f"actually added {actually_added}/{len(feasible_columns)}")
                    print()
                ###
        ...
        return dual_bound, primal_bound, end_CG-start_CG