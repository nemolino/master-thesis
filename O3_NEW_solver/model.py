...
class ModelSolver():
    ...
    def build_model(ins, vtype=None):
        ...
        #print(f"{end-start:.3f} s to build objective function")
        ...
        model.addConstrs(
            (
                gp.quicksum(
                    q_connections_R_bandwidth[u,v]  * x[u,i] * x[v,j]
                    for (i,j) in a2[(l,m)] 
                        for (u,v) in D
    ...
    def optimize_model(instance):
        ...
        if model.Status == GRB.Status.OPTIMAL:
            ...
            #print(f"solve time : {model.Runtime}, TIME_LIMIT, # explored nodes : {model.NodeCount}, z* = {model.ObjVal}, obj_bound = {model.ObjBound} , sol_count = {model.SolCount}")
    ...
    def optimize_model_at_root_node(instance):
        ...
        if model.Status == GRB.Status.OPTIMAL:
            #print(f"solve time : {model.Runtime+presolve_time}, OPTIMAL, z* = {model.ObjVal}")
            return (presolve_time+model.Runtime, "OPTIMAL", model.ObjVal)
        elif model.Status == GRB.Status.INFEASIBLE:
            #print(f"solve time : {model.Runtime+presolve_time}, INFEASIBLE")
    ...
    # used in subMIPing
    @staticmethod
    def optimize_model_restricted(instance, constraints_to_add):
        ...
        shift = 0
        N = len(constraints_to_add)
        for n in range(N):
            T_n = len(constraints_to_add[n])
            for u in range(T_n):
                model.addConstr(
                    gp.quicksum(x[u+shift,i] for i in constraints_to_add[n][u]) >= 1,
                    name=f'restriction_{n}_{u}'
                )
            for u in range(T_n):
                print(n,u,[(u+shift,i) for i in constraints_to_add[n][u]])
            shift += T_n
    ...
    # used in diving with subMIPing
    @staticmethod
    def optimize_model_restricted_2(instance, fixed_columns, app_sizes):
        ...
        for n in range(N):
            if fixed_columns[n] is not None:
                column = fixed_columns[n]
                for u in range(app_sizes[n]):
                    model.addConstr(
                        x[u+shift,column.original_x[u]] == 1,
                        name=f'restriction_{n}_{u}'
                    )
                    print(f"app {n}, x[{u+shift},{column.original_x[u]}] = 1")
    ...
    @staticmethod
    def optimize_model_restricted_3(instance, complete_fixing_history, app_sizes, start_R):
        ...
        for (n,u,i) in complete_fixing_history:
            ...
            #print(f"app {n}, x[{u} -> {u+shift[n]},{i}] = 1")
