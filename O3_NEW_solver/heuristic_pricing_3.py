...
class HeuristicPricing:
    ...
    # def generate_feasible_columns(self, LIMIT=50):
    ...
    #         try:
    #             sol_obtained = self.heuristic_greedy_random_try_solve(deepcopy(cur_sol))
    #         except Exception as e:
    #             continue
    #         feasible_columns.append(sol_obtained)
    #         appended += 1
    #         if appended >= LIMIT:
    #             break
        
        # return feasible_columns
    ...
    def generate_feasible_columns_semigreedy(self):
        ...
        diversification_columns = sorted(columns[50:], key=lambda col: sum(M[u,col.cur_x[u]] for u in range(self.T)))[:150]

        return low_cost_columns + diversification_columns

    """
    def generate_feasible_columns(self):

        cur_sol = deepcopy(self.sol)

        start = perf_counter()
        feasible_diversified_columns = []
        appended = 0
        for _ in range(10000):
            try:
                ...
                continue
            feasible_diversified_columns.append(sol_obtained)
            appended += 1
            if appended >= 400:
                break
        end = perf_counter()
        print(f"len(feasible_diversified_columns) = {len(feasible_diversified_columns)} , generated in {end-start:.2f} s")
        
        start = perf_counter()
        feasible_lowcost_columns = []
        for k in range(2,11):
            appended = 0
            for _ in range(10000):
                try:
                    sol_obtained = self.heuristic_greedy_random_towards_optimality_try_solve(
                        deepcopy(cur_sol), limit_func = lambda x: len(x)//k+1
                    )
                except Exception as e:
                    continue
                feasible_lowcost_columns.append(sol_obtained)
                appended += 1
                if appended >= 100:
                    break
        end = perf_counter()

        print(f"len(feasible_lowcost_columns) = {len(feasible_lowcost_columns)} , generated in {end-start:.2f} s")
        
        return feasible_diversified_columns + sorted(feasible_lowcost_columns, key=lambda x: x.cur_z)[:400]
    

    def generate_feasible_columns_2(self, D):

        cur_sol = deepcopy(self.sol)

        T = self.T
        for u in range(T):
            if len(cur_sol.feasible_nodes[u]) > 0: # se c'era un unico possibile candidato è già stato fissato
                for i in D[u].keys():
                    cur_sol.feasible_nodes[u].remove(i)
                favourite_nodes = sorted(D[u].keys(), key=lambda i: D[u][i], reverse=True)
                cur_sol.feasible_nodes[u] = favourite_nodes + cur_sol.feasible_nodes[u]

        # for u in range(T):
        #     print(f"{u} {cur_sol.feasible_nodes[u]}")
        ...
        return sorted(feasible_lowcost_columns, key=lambda x: x.cur_z)[:200]
    """
    ...
    """
    def heuristic_greedy_random_towards_optimality_try_solve(self, sol, limit_func):
        ...
        while len(sol.cur_x_not_fixed) > 0:
            ...
            # assign_microservice_u_to_random_candidate
            u = min_u
            i = random.choice(sol.feasible_nodes[u][:limit_func(sol.feasible_nodes[u])])
            self.assign_microservice_u_to_node_i(sol, u, i)
    ...
    def insertion_sort(arr):
        for i in range(1, len(arr)):
            key = arr[i] #len(arr[i])
            j = i - 1
            while j >= 0 and key < arr[j]: #len(arr[j])
                arr[j+1] = arr[j]
                j -= 1
            arr[j+1] = key
    """