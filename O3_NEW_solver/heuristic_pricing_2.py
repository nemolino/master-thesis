...
class HeuristicPricing:
    ...
    def generate_feasible_columns(self, LIMIT=50):
        ...
        for idx in range(1000):
            try:
                sol_obtained = self.heuristic_greedy_random_try_solve(deepcopy(cur_sol), verbose=False)
    ...
    def solve(self, λ_positive, μ_positive, η_n):
        ...
        for idx in range(100):
            ...
            try:
                if idx == 0:
                    sol_obtained = self.heuristic_greedy_try_solve(deepcopy(cur_sol), verbose=False)
                else:
                    sol_obtained = self.heuristic_greedy_random_try_solve(deepcopy(cur_sol), verbose=False)
            except Exception as e:
                continue
            ...
            if sol_obtained.cur_z < -0.001:
                found = True
                break
        
        
        #print("HeuristicSingleStepLookAhead riesce")
        return sol_obtained if found else None
    

    def solve_multiple(self, λ_positive, μ_positive, η_n, how_many_max):
        ...
        for idx in range(how_many_max):
            ...
            if sol_obtained.cur_z < -0.001:
                sols_obtained.append(sol_obtained)
                #found = True
                #break
        
        
        #print("HeuristicSingleStepLookAhead riesce")
        return sols_obtained if len(sols_obtained)>0 else None
    ...
    def assign_microservice_u_to_random_candidate(self, sol, u):
    ...
    def heuristic_greedy_random_try_solve(self, sol, verbose=True):
        ...
        while True:
            ...
            self.assign_microservice_u_to_random_candidate(sol, min_u)
            self.preprocessing(sol)
            #print(f"... fixing microservice {min_u} because it has the minimum number of candidates\n")
            #CUR_Z = fix_microservice(FEASIBLE_NODES, min_u, CUR_Z, CUR_X, d, residual_CORE, residual_BANDWIDTH)
