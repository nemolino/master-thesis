...
class ColumnGeneration:
    ...
    def execute(self):
        ...
        # not_still_to_fix = []
        # for n in range(N):
        #     not_still_to_fix.append(set())
        
        # for n in range(N):
        #     print(n, still_to_fix[n]) #, not_still_to_fix[n])
        # print(fixed_variables_count)
        # print()
        ...
        # def stop(N, T_n, fixed_variables_count, total_T):
        #     if sum(fixed_variables_count) >= 0.6 * total_T:
        #     return True
        #     # for n in range(N):
        #     #     if fixed_variables_count[n] < floor(T_n[n]*0.7):
        #     #         return False
        #     return True
        ...
        while not sum(fixed_variables_count) >= 0.7 * total_T:

            ### trovo la variabile originale con valore frazionario più vicino a 1 e la fisso a 1

            # for n in range(N):
            #     print(f"app {n}, to fix {len(still_to_fix[n])} / {T_n[n]}") #, !fixed = {still_to_fix[n]}, fixed = {not_still_to_fix[n]}")
            ...
            #not_still_to_fix[n_most].add(u_most)

            complete_fixings_history.append((n_most,u_most,i_most))
            #relevant_fixings_history.append((n_most,u_most,i_most))
            ...
            while True:
                ...
                try:
                    λ_positive, μ_positive, η, master_obj_val = master.optimize()
                    #print(λ_positive, μ_positive, η, master_obj_val)
                except InfeasibleMaster:
                    end_R = perf_counter()
                    print(f"\nINFEASIBLE MASTER, return {end_R-start_R:.3f}")
                    return 
                    #infeasible_master = True #infeasible_master = False
                    #break
                    #print("STOP")
                    #return best_dual_bound, "... primal_bound ...", "... tempo ..." 
                ...
                for n in range(N):
                    ...
                    #print(f"... solving pricing of {n}-th application")
            ...
            if master.solution_is_integer(N):
                ...
                end_R = perf_counter()
                print(f"rounding_subMIPing SUCCESS perchè CG_ottimo_intero, primal bound {master.model.ObjVal}, return {end_R-start_R:.3f}")
                return #best_dual_bound, primal_bound, end_CG-start_CG
        ...
        res = ModelSolver.optimize_model_restricted_3(instance, complete_fixings_history, T_n, start_R)
        end_R = perf_counter()
        print(f"{end_R-start_R:.3f} result {res}")
        return 

        # print(f"result {res} time {end-start:.3f}\n")"
